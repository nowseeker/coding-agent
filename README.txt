...
特色功能：使用模型原生 tool calling，自主读取/搜索/修改工作区文件并执行本地命令；支持终端多轮会话、严格请求预算、工具语义压缩、代码结构与行号分析、严格工具参数校验、证据完成门、每项目本地 API 轮次审计、API 重试、命令超时与输出截断、路径越界和危险命令拦截、重复调用熔断。结构摘要只用于定位，修改前会读取精确代码。对话历史仅在当前进程内保存。API key 仅从环境变量读取，且不会传给子进程。main 分支不包含 Web UI。
