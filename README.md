...
- 提供基于 Tkinter 的本地桌面窗口，可创建和选择项目、保存对话并查看实时工具步骤。
- API Key 只从环境变量读取，不写入仓库，也不会传给工具子进程。

桌面窗口只是现有 `CodingAgent` 的输入输出层，不复制 Agent 循环，也不启动 Web 服务。
...
先在 PowerShell 中设置环境变量，然后运行：

```powershell
python ui.py
```

窗口左侧可以创建或选择项目，中间显示对话历史和工具执行进度。默认项目目录为
`workspaces/`，项目与对话状态保存在 `.coding-agent/desktop-ui-state.json`，API 审计记录
保存在 `.coding-agent/api-traces/`；这些路径均已被 Git 忽略。状态文件可能包含用户任务和
回答，应只在本机保存。

窗口打开期间，当前 PowerShell 会等待窗口关闭，这是桌面程序的正常行为。需要继续使用终端时，
请另开一个 PowerShell 窗口。安装项目后也可以运行：

```powershell
coding-agent-ui
```

桌面 UI 不提供 API Key 输入框，不保存 Key；环境变量必须在启动窗口之前设置。
...
一次任务完成后，CLI 不退出，而是保存该任务与最终回答，再等待下一条用户消息。桌面 UI
则把项目和完成的对话保存在本地状态文件中，重启后可以继续。两种入口都只把完整成功的
用户/助手消息对传给模型；失败轮次会显示，但不会进入后续模型上下文。
